...
public class AESCipherUtil {
    ...
    /**
     * 加密算法/加密模式/填充类型。采用AES加密，ECB加密模式，PKCS5Padding填充
     */
    private static final String CIPHER_MODE = "AES/ECB/PKCS5Padding";

    /**
     * 设置iv偏移量。采用ECB加密模式，不需要设置iv偏移量
     */
...
//    }
     ...
     * @return 返回密文，加密后得到的内容。加密错误返回null
     */
    public static String encryptToHex(String clearText, String password) {
        try {
            // 1 获取加密密文字节数组
            byte[] cipherTextBytes = encrypt(clearText.getBytes(CHARACTER), pwdHandler(password));
    ...
    public static String getSecret() {
        String secret = System.getProperty("secret", "txjRuIWEDDuv");
        return secret + ">+<SGWGYSUXNOJM8TJ3NXNI";
    }

    public static void main(String[] args) throws Exception {
        String address = encryptToHex("test", AESCipherUtil.getSecret());
        System.out.println(address);

        String password = decryptFromHex("6490673AEE2330B2E9866621F8C925C215F65EFE86181EC11750896234ED22F97D8CD79CFDD6A9B0E8A1ACAF3A23630B", AESCipherUtil.getSecret());
        System.out.println(password);
        System.err.println(AESCipherUtil.getSecret());
