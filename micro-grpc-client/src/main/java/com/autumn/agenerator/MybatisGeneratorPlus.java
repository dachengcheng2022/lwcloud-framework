 ...
 */
public class MybatisGeneratorPlus {

    private static final Boolean CREATE_CONTROLLER = Boolean.FALSE;
    private static final Boolean CREATE_SERVCIE = Boolean.TRUE;
    private static final Boolean CREATE_MAPPER = Boolean.TRUE;
    private static final Boolean CREATE_PROVIDER = Boolean.TRUE;
    private static final Boolean CREATE_DOMAIN = Boolean.TRUE;
    /**
     * 微服务名
     */
    private static String SERVICE_NAME = "micro-user-server";
     ...
     */
    private static String PARENT_PACKAGE_PATH = "/com/autumn";
    ...
    private static String password = "eaa45a4431ae221f";
    private static String url = "jdbc:mysql://120.76.176.30:3306/kmcksy-mall?serverTimezone=GMT%2B8&characterEncoding=UTF-8&allowMultiQueries=true";
    ...
    final static String FRAME_MODULE = "";
    final static String PROJECT_PATH = System.getProperty("user.dir") + File.separator + FRAME_MODULE + File.separator + SERVICE_NAME;
                ...
                .setPassword(password).setTypeConvert(new MySqlTypeConvert() {
                    ...
                    public IColumnType processTypeConvert(GlobalConfig config, String fieldType) {
                        return use(fieldType)
                                ...
                                .test(containsAny("tinyint", "bit").then(DbColumnType.INTEGER))
    ...
    private static List<FileOutConfig> fileOutConfigList() {
        ...
        // 当前项目路径
        final String projectPath = System.getProperty("user.dir") + "/" + FRAME_MODULE + File.separator + SERVICE_NAME;
        ...
        if(CREATE_MAPPER){
            ...
            list.add(new FileOutConfig(XML_TEMPLATE) {
                @Override
                public String outputFile(TableInfo tableInfo) {
                    return projectPath + XML_OUTPUT_PATH + MODULE_NAME + "/" + tableInfo.getMapperName() + StringPool.DOT_XML;
                }
            });
        }
        ...
        if(CREATE_PROVIDER){
            //         provider java文件输出
            list.add(new FileOutConfig(PROVIDER_TEMPLATE) {
                @Override
                public String outputFile(TableInfo tableInfo) {
                    return projectPath + PROVIDER_OUTPUT_PATH + MODULE_NAME + "/" + tableInfo.getEntityName() + "SqlProvider" + StringPool.DOT_JAVA;
                }
            });
        }
        ...
        if(CREATE_MAPPER){
            list.add(new FileOutConfig(MAPPER_TEMPLATE) {
                @Override
                public String outputFile(TableInfo tableInfo) {
                    return projectPath + MAPPER_OUTPUT_PATH + MODULE_NAME + "/" + tableInfo.getMapperName() + StringPool.DOT_JAVA;
                }
            });
        }
        ...
        if(CREATE_SERVCIE){
            ...
            list.add(new FileOutConfig(SERVICE_TEMPLATE) {
                @Override
                public String outputFile(TableInfo tableInfo) {
                    return projectPath + SERVICE_OUTPUT_PATH + MODULE_NAME + "/" + tableInfo.getServiceName() + StringPool.DOT_JAVA;
                }
            });
        ...
        list.add(new FileOutConfig(CONTROLLER_TEMPLATE) {
            @Override
            public String outputFile(TableInfo tableInfo) {
                return projectPath + CONTROLLER_OUTPUT_PATH + MODULE_NAME + "/" + tableInfo.getControllerName() + StringPool.DOT_JAVA;
            }
        });
        }
        return list;
